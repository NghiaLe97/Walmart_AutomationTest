    ...
    if getattr(sys, 'frozen', False):
        ...
        for item in base_dir.iterdir():
            try:
                ...
                elif item.is_file() and item.name != Path(sys.argv[0]).name:
                    shutil.copy2(item, temp_dir)
            except PermissionError as e:
                print(f"PermissionError: {e} - Skipping {item}")
...
def get_base_dir():
    # Trả về thư mục tạm khi chạy từ tệp .exe, hoặc thư mục gốc của mã nguồn
    if getattr(sys, 'frozen', False):
        return Path(sys._MEIPASS)
    else:
        return Path(__file__).resolve().parent

# Sử dụng hàm này để xác định đường dẫn đến tệp Excel
base_dir = get_base_dir()
document_path = base_dir / "database" / "Mode01_41_document.xlsx"
        ...
        def process_cases():
            for case_name in selected_cases:
                ...
                if sim_file_path:
                    ...
                    self.root.after(0,
                                    lambda name=case_name: self.update_result(name, "Not Found", "N/A", "N/A"))
