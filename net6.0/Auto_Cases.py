...
class TestApp(unittest.TestCase):
    ...
    device_id = "172.32.0.152:38759"  # ID thiết bị
    ...
    def Auto(self, retries=3):
        for attempt in range(retries):
            try:
                ...
                # Check vehicle, select YMME
                success, message = Actions.check_vehicle_selection(self)
                if not success:
                    print(f"Test failed: {message}")
                sleep(15)
                ...
                success = Actions.check_DTC(self)
                if not success:
                    print(f"Test failed: {success}")
                self.update_setting("DTC", success)
                sleep(5)

                success = Actions.check_FF(self)
                if not success:
                    print(f"Test failed: {success}")
                self.update_setting("Freeze Frame", success)
                sleep(5)

                success = Actions.check_MIL(self)
                if not success:
                    print(f"Test failed: {success}")
                self.update_setting("MIL", success)
                sleep(5)
